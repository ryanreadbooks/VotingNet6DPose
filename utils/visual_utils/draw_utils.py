...
def draw_linemod_mask_v2(mask: np.ndarray) -> Image:
    onehot_mask = mask_onehot_converter(mask, regular_config.num_of_class)
...
def draw_vector_field(field: np.ndarray) -> np.ndarray:
    ...
    :param field: the vector field of shape (2, h, w) with channel order (x, y)
    :return: vector field image array of shape (h, w, 3), the opencv array format
    """
...
def visual_vector_field(field: np.ndarray):
    """
    Visual the vector field in a more beautiful way
    @param field: the the vector field of shape (2, h, w) with channel order (x, y)
    @return:
    """
    # compute the angle at each pixel location in degree
    angle_map = np.arctan2(field[1], field[0]) * (180.0 / np.pi)  # shape (h, w)
    return cv2.applyColorMap(np.array(angle_map, dtype=np.uint8), cv2.COLORMAP_HSV)
