...
from utils import draw_linemod_mask_v2, draw_3d_bbox, draw_points
...
if __name__ == '__main__':
	...
	for data in test_dataloader:
		...
		# 将mask二值化用来显示
		binary_mask = torch.where(binary_mask == 4, torch.tensor(255).to(device), torch.tensor(0).to(device))
		...
		cat_vector_map: np.ndarray = LinemodOutputExtractor.extract_vector_field_by_name(pred_vector_map[0], 'cat')
