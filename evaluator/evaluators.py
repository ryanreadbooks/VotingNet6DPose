...
class LinemodEvaluator(object):
    def __init__(self, network: torch.nn.Module, refinement: bool = False, simple: bool = False,
        ...
        model_path = os.path.join(regular_config.data_path_root, category, category + '.xyz')
        ...
        self.voting_procedure = VoteProcedure((regular_config.img_height, regular_config.img_width))
        ...
        for i, data in enumerate(dataloader):
            ...
            for j in range(batch_size):
                ...
                if self.refinement is not None:
                    ...
                    proj_error_refined: float = metrics.projection_error(pts_3d=self.model_points, camera_k=regular_config.camera,
                ...
                proj_error: float = metrics.projection_error(pts_3d=self.model_points, camera_k=regular_config.camera,
                                                             pred_pose=pred_pose,
                                                             gt_pose=gt_pose)
    ...
    def pipeline(self, img_path: str) -> np.ndarray:
        ...
        if regular_config.result_path == '':
            ...
            mask_save_path = os.path.join(regular_config.result_path, 'predicted_{}_mask.png'.format(self.category))
        ...
        # save the result
        box_save_path = regular_config.result_path + '/predicted_{}_3dbox.png'.format(self.category)
