...
class VoteProcedure(object):
	...
	def _generate_candidates_from_population(self, population: List, vectors: np.ndarray) -> np.ndarray:
		...
		while True:
			...
			if not (slope1 == slope2):
				...
				valid = self.check_intersection_validity(point1, point2, vector1, vector2, intersection_x, intersection_y)
				if valid:
	...
	def provide_keypoints(self, mask: np.ndarray, vmap: np.ndarray, threshold: float = 0.99) -> np.ndarray:
		...
		keypoints_dict: Dict[int, np.ndarray] = self._vote_average(temp_keypoints)
		keypoints = np.array(list(keypoints_dict.values()))
		return keypoints

	def check_intersection_validity(self, p1: Tuple, p2: Tuple, v1: np.ndarray, v2: np.ndarray, x: float, y: float) -> bool:
		"""
		...
		if not (0 <= x <= self.width and 0 <= y <= self.height):
			...
			if d1 @ v1.reshape((2, 1)) >= 0.99 and d2 @ v2.reshape((2, 1)) >= 0.99:
				return True
		return False
